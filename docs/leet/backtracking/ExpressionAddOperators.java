 ...
 * (not unary) +, -, or * between the digits so they evaluate to the target value.
 ...
 Solution: Backtrack and keep track of the total and product value.
 In case of + or - add/sub curr to total and curr becomes the new product
 In case of * take difference of total and prod and add (product of curr value with previous product and make this a
 new product for the next iteration)

 Worst-case time complexity can be O(n * (2^n-1))
 */
public class ExpressionAddOperators {
    ...
    private void backTrack(String exp, List<String> list, int curr, String num, int target, long total,
                           long prod){
        if(curr == num.length()){
            if(total == target){
                list.add(exp);
            ...
            for(int i = curr, l = num.length(); i < l; i ++){
                ...
                if(curr == 0){
                    ...
                    backTrack(exp + "*" + newNum, list, i + 1, num, target, (total - prod  + (prod * newNumL)), prod
                            * newNumL);
                }
            }
        }
