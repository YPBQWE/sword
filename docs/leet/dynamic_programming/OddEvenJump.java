...
import java.util.*;
/**
 * Created by gouthamvidyapradhan on 17/03/2019
 * You are given an integer array A.  From some starting index, you can make a series of jumps.  The (1st, 3rd, 5th,
 * ...) jumps in the series are called odd numbered jumps, and the (2nd, 4th, 6th, ...) jumps in the series are
 ...
 *
 * Solution: O(N log N)
 * For each array index and for each odd/even turn pre-calculate the next jump index - this can be achieved in O(n
 * log n) by using a balanced tree.
 * Check for each array index if we can reach end of the array by using the pre-calculated values for next jump -
 * cache the values to avoid recalculating. Sum up total number of such start indices and that will be the answer.
 */
